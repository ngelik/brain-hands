...
npm run verify:focused -- tests/path/to/changed-surface.test.ts
```

The focused verifier typechecks, builds once, freezes the `dist/` digest, runs
only the named test files under `BRAIN_HANDS_DIST_IMMUTABLE=1`, and checks the
digest again. It is intended for bounded edit/fix loops and is not release
evidence.
